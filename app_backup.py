...
def add_vitals():
    ...
    # 2. DECISION LOGIC & INCIDENT RESPONSE
    status = "Stable"
    ...
    if ai_risk == "High":
        ...
        # --- PHASE 18: SIMULATED EMAIL DISPATCH ---
        print(
            f"\n[SERVER LOG] 📧 SMTP WORKER: Sending Urgent Email to dr.bhatt@vitalmine.hospital"
        )
        print(
            f"[SERVER LOG] 📝 SUBJECT: CRITICAL SEPSIS ALERT - Patient {patient_name}"
        )
        print(
            f"[SERVER LOG] 📎 PAYLOAD: Temp={temp}, HR={hr} >> Dispatched Successfully.\n"
        )

        flash(
            f"🚨 EMERGENCY PROTOCOL INITIATED: Critical Alert sent to On-Call Physician regarding {patient_name}.",
            "danger",
        )
    ...
    # Priority 2: WARNINGS
        ...
        flash(f"⚠️ High Fever Warning logged for {patient_name}.", "warning")
    elif temp < 36.0:
        status = "Warning"
        advice_text = "Hypothermia Risk. Keep patient warm."
        flash(f"⚠️ Hypothermia Warning logged for {patient_name}.", "warning")
    elif hr > 100:
        status = "Warning"
        advice_text = "Tachycardia (High Heart Rate). Rest and re-check."
        flash(f"⚠️ Tachycardia Warning logged for {patient_name}.", "warning")
    elif rr > 22:
        status = "Warning"
        advice_text = "Hyperventilation detected. Monitor breathing."
        flash(f"⚠️ Respiratory Warning logged for {patient_name}.", "warning")

    # Priority 3: STABLE
    else:
        status = "Stable"
        flash(f"✅ Vitals logged successfully for {patient_name}.", "success")
